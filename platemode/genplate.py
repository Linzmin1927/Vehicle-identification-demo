...
class GenPlate:
    ...
    def __init__(self,fontCh,fontEng,NoPlates):
        ...
        self.bg  = cv2.resize(cv2.imread(GEN_ROOT+"/images/template.bmp"),(226,70));
        self.smu = cv2.imread(GEN_ROOT+"/images/smu2.jpg");
        self.noplates_path = [];
    ...
    def genBatch_save(self, batchSize,pos,charRange, outputPath,size):
        if (not os.path.exists(outputPath)):
            os.mkdir(outputPath)
        for i in range(batchSize):
            plateStr = self.genPlateString(-1,-1)
            # print(plateStr,type(plateStr))
            img =  self.generate(plateStr);
            img = cv2.resize(img,size);
            filename = os.path.join(outputPath, str(i).zfill(4) + '-' + plateStr+ ".jpg")
            # print(filename,type(filename))
            # cv2.imwrite(filename, img); #不支持中文，使用 cv2.imencode('.jpg',src)[1].tofile(filename) 替代
 ...
 # G = GenPlate("./platemode/font/platech.ttf",'./platemode/font/platechar.ttf',"./platemode/NoPlates")
# G.genBatch(2,2,range(31,65),"./platemode/plate_test/",(272,72))
...
G = GenPlate(GEN_ROOT+"/font/platech.ttf",GEN_ROOT+'/font/platechar.ttf',GEN_ROOT+"/NoPlates")
l_plateStr,l_plateImg = G.genBatch(100,2,range(31,65),(272,72))
...
if False:
    ...
    cv2.imshow('Image', np.vstack(l_out))
    cv2.waitKey()
